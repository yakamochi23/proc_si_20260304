...
\begin{document}
    ...
    \section{評価実験}
        ...
        \begin{table*}[tb]
            ...
            \caption{aiueo}
            \label{tab:result}
            \begin{tabular*}{\linewidth}{c|cccccccc}
                \hline
                Method & $\mathrm{MOTA} \uparrow$ & $\mathrm{|FP| \downarrow}$ & $\mathrm{|FN| \downarrow}$ & $\mathrm{|IDs| \downarrow}$ & $\mathrm{|IDF1| \uparrow}$ & $\mathrm{|IDP| \uparrow}$ & $\mathrm{IDR} \uparrow$ & $\mathrm{|FM| \downarrow}$\\
                \hline
                \hline
        ...
        \subsection{実験結果および考察}
            表\ref{tab:result}に，VIP-HTDデータセットのテストデータに対する定量評価結果を示す．

            図\ref{fig:result_vis}に，定性評価結果を示す．

                \begin{figure*}[tb]
                    \centering
                    \includegraphics[width=\linewidth]{figures/section4/result_vis.png}
                    \caption{VIP-HTDのCGY\_VS\_DAL\_003シーケンスに対する追跡結果の例．}
