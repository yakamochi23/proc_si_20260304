    ...
    \section{提案手法}
        ...
        \subsection{姿勢情報の更新}
            式\ref{eq:final_cost}のコスト関数$\mathcal{L}_{\mathrm{match}}(\cdot)$を用いたAssociationにより，トラック$\mathbf{x}_{i}$と検出結果$\mathbf{y}_{j}$が対応付けられたとする．このとき，$\mathbf{x}_{i}$が保持する姿勢情報$\{ \mathbf{g}_{i}, c_{i} \}$は式\ref{eq:update_pose_info}によって更新させる．
                ...
                \end{equation}
            これは，．一方，どの検出結果にも対応付けられなかったトラック$\mathbf{x}_{k}$に関しては，$\mathbf{x}_{i}$が保持する姿勢情報$\{ \mathbf{g}_{k}, c_{k} \}$は式\ref{eq:decay_pose_conf}によって更新させる．

            姿勢特徴量$\mathbf{g}_{k}$の更新は行わずに，姿勢信頼度$c_{k}$を式\ref{eq:decay_pose_conf}によって減衰させる．
                \begin{equation}
