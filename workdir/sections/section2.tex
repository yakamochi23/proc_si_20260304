...
\begin{document}
    ...
    \section{関連研究}
        ...
        DeepSORT\cite{DeepSORT2017}では，SORTの課題を克服することを目的に，IoUに加えて物体の外観情報をAssociationに利用している．具体的には，各検出物体のBBox(Bounding-Box)に対応する画像領域から，深層学習に基づく再同定器によって物体の外観特徴量を抽出し，その特徴量の類似性に基づいて既存トラックと検出結果を対応付ける．これにより，複雑な動きを示す物体に対しても頑健な物体追跡を実現している．しかし，物体の外観類似性に基づくAssociationは必ずしもMOTに有効であるとは限らない．例えば，外観識別性の高い物体が登場するシーン\cite{MOT16}においてはDeepSORTが高い追跡精度を示す一方で，選手が共通のユニフォームを着用するスポーツシーン\cite{DanceTrack2022}や，野生動物を追跡対象とするシーン\cite{AnimalTrack2022}では，その追跡精度がSORTよりも劣ることが報告されている．

        近年，外観情報を用いずに運動情報を強化することで追跡精度を向上させる手法も提案されている．例えばOC-SORT\cite{OC-SORT2023}は，．
        
        また，姿勢情報をAssociationに利用した手法も提案されている．例えば，姿勢推定器が出力する物体の関節点（keypoint）に対して，Pose Flow\cite{}はkeypointのOKS(Object Keypoint Similarity)によって，Light Track\cite{}はkeypointをSGCN(Siamese Graph Convolutional Network)
        に入力して得られるグラフ特徴量の類似性によってコスト関数を設計している．
        これらの手法は，keypointによって物体の身体構造を捉え，外観が類似する物体に対しても識別性を強化している．しかし，スポーツシーンでは，選手同士の接触プレーによってオクルージョンが頻発し，keypointの欠落や誤推定が生じやすい．その結果，算出される姿勢類似度が不安定になり，Associationの精度が低下する恐れがある．さらに，keypointベースの追跡手法は，高品質な姿勢推定器に依存することから，対象ドメインに対する十分な姿勢推定器の訓練が必要となるが，keypointのアノテーションを必要とする，といった実運用上の制約も存在する．
        
        本研究では，姿勢情報の有用性を活かしつつ，keypointの完全性に強く依存しない表現として，姿勢推定器から得られる中間特徴量の類似性に基づいたAssociationを提案する．これにより，スポーツ特有の「外観識別性にの低い物体群が複雑に動き，オクルージョンが多発する」環境においても安定した物体追跡を目指す．
