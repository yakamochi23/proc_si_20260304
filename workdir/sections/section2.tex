...
\begin{document}
    ...
    \section{関連研究}
        ...
        また，姿勢情報をAssociationに利用した手法も提案されている．例えば，姿勢推定器が出力する推定関節点（keypoint）に対し，Pose Flow\cite{}はOKS（Object Keypoint Similarity）に基づく類似度を用い，Light Track\cite{}はkeypointをSGCN（Siamese Graph Convolutional Network）に入力して得たグラフ特徴量の類似度によりコスト関数を設計している．
        これらの手法は，keypointによって身体構造を捉えることで，外観が類似する物体に対しても識別性を強化している．
        しかし，スポーツシーンでは，選手同士の接触や交錯によるOcclusionが頻発し，keypointの欠落・誤推定が生じやすい．その結果，姿勢類似度が不安定となり，Associationの精度が低下する恐れがある．さらに，keypointベースの追跡は，高精度な姿勢推定器に依存するため，対象ドメイン向けの学習が必要となり，keypointアノテーションのコストという実運用上の制約も伴う．
